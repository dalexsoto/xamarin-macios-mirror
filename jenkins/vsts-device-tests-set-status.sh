...
if test -n "$START"; then
	...
	case "$(echo "$AGENT_JOBSTATUS" | tr '[:upper:]' '[:lower:]')" in
		...
		failed | canceled | succeededwithissues | *)
			GH_STATE=error
			DESCRIPTION="Device tests failed ($AGENT_JOBSTATUS)"
			RESULT_EMOJII="🔥 "
			;;
	esac
fi
...
if test -z "$START"; then
	...
	printf "%s%s on [Azure DevOps](%s)\\n\\n" "$RESULT_EMOJII" "$DESCRIPTION" "$VSTS_BUILD_URL" > "$MESSAGE_FILE"
	printf "%s[Html Report](http://xamarin-storage/%s/jenkins-results/tests/index.html)\\n" "$RESULT_EMOJII" "$P" >> "$MESSAGE_FILE"
